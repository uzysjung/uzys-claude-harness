...
- **처음 설치** → 단계별 안내: Track 선택 → Tauri / GSD / ECC / Trail of Bits 옵션 → 요약 → 확인
- **기존 설치 감지** → 5-메뉴 표시:
  1. 새 Track 추가
  2. 정책 파일 업데이트 (백업 자동)
...
CI/CD나 스크립트 환경은 기존 플래그 그대로 작동 — `--track` 명시하면 인터랙티브 skip:
...
비-TTY 환경 + `--track` 누락 시 `--track required in non-interactive mode` 에러로 종료.

이 한 줄이 하는 일:
1. 하네스를 임시 디렉토리에 shallow-clone
2. `claude-harness install` 실행 (인터랙티브 or 플래그)
3. 임시 디렉토리 정리
4. 설치된 Track을 `.claude/.installed-tracks`에 기록 (다음 설치 시 자동 감지용)
