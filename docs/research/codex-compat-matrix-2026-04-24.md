...
> **Date**: 2026-04-24 (초안), 2026-04-24 (실측 revision v2)
> **Source**: Context7 + Codex 0.124.0 실측 (`codex features list`) + `stellarlinkco/codex/docs/hooks.md` + Issue #16732/#17532
> **Linked SPEC**: `docs/specs/codex-compat.md`
>
> **Revision 요지**: 초안 §1~§5는 Context7 `0.29/0.75` 문서 기준이었다. Codex 0.124.0 실측 결과 Hook/Plugin/Multi-agent가 전부 stable이며 Claude Code와 거의 1:1 대응. §7에 실측 섹션 추가.
...
## 7. 실측 Revision (Codex 0.124.0 — 2026-04-24)

### 7.1 Feature flags (`codex features list`)

| Feature | Stage | Default | 의미 |
|---------|-------|---------|------|
| `codex_hooks` | **stable** | **true** | Hook 시스템 공식 지원 — §3 갭 전제 폐기 |
| `multi_agent` | stable | true | `spawn_agent/wait_agent/close_agent` 공식 툴. Claude `context: fork` 대응 |
| `plugins` | stable | true | Plugin 시스템 (`.codex-plugin/plugin.json` + hooks.json + skills) |
| `tool_search` | stable | true | Claude ToolSearch 대응 |
| `skill_mcp_dependency_install` | stable | true | Skill이 MCP 자동 설치 |
| `child_agents_md` | under dev | **false** | AGENTS.md 디렉토리 계층 merge. **OQ1 기본 비활성 확정** |
| `memories` | experimental | false | Claude auto memory 유사 |
| `codex_git_commit` | under dev | false | git commit 자동화 (미완) |
| `apps` | stable | true | 앱/커넥터 시스템 |
| `fast_mode` | stable | true | — |

### 7.2 Hook 이벤트 맵 (Claude ↔ Codex)

| Claude Hook | Codex Event | Blockable | 비고 |
|-------------|-------------|-----------|------|
| `SessionStart` | `session_start` | ❌ | 동일 의미 |
| `UserPromptSubmit` | `user_prompt_submit` | ✅ exit 2 | matcher 무시 |
| `PreToolUse` | `pre_tool_use` | ✅ exit 2 | **Bash 한정 발화 (Issue #16732)** |
| `PostToolUse` | `post_tool_use` / `post_tool_use_failure` | ❌ | 실패 전용 별도 이벤트 |
| `Stop` | `stop` | ✅ | subagent_stop 별개 |
| (Claude 없음) | `permission_request` | ✅ | Codex 네이티브 승인 요청 |
| (Claude 없음) | `pre_compact` | ❌ | 컨텍스트 압축 직전 |
| (Claude 없음) | `worktree_create/remove` | mixed | multi-agent 용 |

### 7.3 stdin/stdout 프로토콜 호환성

Claude Code hook 스크립트가 Codex에서도 **대부분 그대로 작동 가능**:
...
| `tool_name` | ✓ | ✓ | ✅ |
| `tool_input` | ✓ | ✓ | ✅ |
| `hook_event_name` | PascalCase | PascalCase | ✅ (예: `SessionStart`) |
| `permission_mode` | ✓ | ✓ (값 도메인 차이 가능) | ⚠ |
| stdout `decision` | allow/deny/ask | allow/deny/ask | ✅ |
| stdout `continue` | bool | bool | ✅ |
| stdout `updatedInput` (PreToolUse) | ✓ | `updated_input` snake_case 호환 | ✅ |
| Exit 2 block | ✓ | ✓ | ✅ |

### 7.4 치명적 제약 (upstream 이슈)

| Issue | 영향 | 회피 |
|-------|------|------|
| **#16732** | `pre_tool_use/post_tool_use`가 Bash에만 발화, ApplyPatch(파일 쓰기) 미지원 | `sandbox_mode=workspace-write` + `approval_policy=on-request`로 파일 보호를 네이티브 경로로 이관 (ADR-002 v2 D3) |
| **#17532** | 프로젝트 `./.codex/config.toml` hook이 인터랙티브 세션에서 로드 안 됨. `codex exec`는 정상 | Phase F 실측 확인. 영향 시 자동화 경로(`codex exec`)만 운영 |

### 7.5 ADR-002 수정 반영

§3 "핵심 갭" 판정은 v1 초안 기준. **v2(실측 기반)에서 갭 거의 소멸**:

| 항목 | v1 판정 | v2 판정 |
|------|---------|---------|
| SessionStart | LLM-DEP | ✅ 결정론적 |
| PreToolUse (Skill) | LLM-DEP | ✅ 결정론적 |
| PreToolUse (Write) | sandbox | ✅ **sandbox 네이티브 유지** (Issue #16732 회피) |
| PostToolUse | [notify] 한정 | ✅ `post_tool_use` (Bash 한정 조건부) |
| UserPromptSubmit | shell wrapper | ✅ 결정론적 |
