...
프로젝트별 MCP 서버 목록. `setup-harness.sh`가 Track에 맞게 동적 생성:
...
- **활성화**: `.mcp-allowlist` 파일 존재 시 (opt-in). `scripts/setup-harness.sh`가 설치 시 `.mcp.json` 서버 목록에서 자동 생성
...
- **활성화**: `bash scripts/setup-harness.sh --track <track> --model-routing on` (기본 `off`)
...
```bash
bash scripts/setup-harness.sh --track tooling --project-dir .       # 첫 설치
bash scripts/setup-harness.sh --add-track csr-fastapi --project-dir .  # Track 추가
```
...
```bash
# 1. 프로젝트 초기화
bash scripts/setup-harness.sh --track csr-fastapi
...
```bash
# 1. 프로젝트 초기화
bash scripts/setup-harness.sh --track executive
...
bash scripts/setup-harness.sh --track tooling
