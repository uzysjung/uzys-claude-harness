...
```bash
claude-harness install --track tooling --track csr-fastapi --project-dir .
```
...
```bash
claude-harness install --track tooling --project-dir .              # 첫 설치
claude-harness install --add-track csr-fastapi --project-dir .       # Track 추가
```
...
```bash
# 1. 프로젝트 초기화
claude-harness install --track csr-fastapi
...
```bash
# 1. 프로젝트 초기화
claude-harness install --track executive
...
```bash
# 1. 프로젝트 초기화 (Claude baseline + OpenCode 자산)
claude-harness install --track tooling --cli opencode --project-dir .
...
```bash
claude-harness install --track tooling --cli all --project-dir .
