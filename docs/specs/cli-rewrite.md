...
- **AC4 — 테스트**: Vitest 테스트 ≥ 30개 (단위 + 통합). 커버리지 ≥ 80% (CLI 본체).
...
- **OQ4 — `claude-to-codex.sh` TS 포팅 여부**: 247줄 작은 스크립트. (a) TS로 포팅(일관성) (b) 그대로 호출(작업량 절감). → Phase D 진입 시 결정.
...
- **Phase E — 테스트 스위트** (F8): Vitest 단위 + 통합. 9 Track 통합 테스트 (mock fs 또는 실 tmp dir). 커버리지 80%. (1일)
...
본 SPEC Status: Draft. 아래 항목 동의 시 Status: Draft → Accepted 전환 + Phase A 착수:

1. Sprint Contract (§1 Objective + §2 AC + §3.2 Non-Goals) 동의?
2. Phase 분해 (§4, 5~7일) 동의?
3. OQ1~OQ6 중 **지금 결정**할 항목? (특히 OQ1 npm vs GH Releases는 분배 영향 큼)
4. ADR-003 (언어 선택 + 분배 전략) 동시 작성 동의?

승인하면 ADR-003 드래프트 작성 + Phase A 진입 (다음 세션부터).
