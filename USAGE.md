...
| `ecc:` | ECC cherry-pick | 보안 스캔, 경험 관리 |
| `imm:` | Impeccable | 디자인 품질 (17개 스킬) |
...
| `/imm:audit` | 접근성/성능/반응형 체크 |
| `/imm:adapt` | 반응형 디자인 적응 |
| `/imm:animate` | 모션/마이크로 인터랙션 |
| `/imm:bolder` | 디자인 임팩트 강화 |
| `/imm:clarify` | UX 카피/에러 메시지 개선 |
| `/imm:colorize` | 전략적 색상 추가 |
| `/imm:delight` | 개성/터치 추가 |
| `/imm:distill` | 불필요한 복잡성 제거 |
| `/imm:layout` | 레이아웃/간격/리듬 개선 |
| `/imm:optimize` | UI 성능 최적화 |
| `/imm:overdrive` | 기술적으로 야심찬 구현 |
| `/imm:quieter` | 과도한 디자인 톤다운 |
| `/imm:typeset` | 타이포그래피 개선 |
